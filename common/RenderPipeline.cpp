...
RenderPipeline::RenderPipeline() {
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, drawWidth, drawHeight);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depthBuffer);
    
    // Allocate albedo texture to render to.
    glGenTextures(1, &colorTexture);
    glBindTexture(GL_TEXTURE_2D, colorTexture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, drawHeight, drawHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
...
uint8_t * RenderPipeline::RayTracePixel(float x, float y, bool geometry) {
    ...
    if(geometry) {
        ...
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depthBuffer);
        xpixel = clamp((int) floor(x * drawWidth), 0, drawWidth - 1);
        ypixel = clamp((int) floor(y * drawHeight), 0, drawHeight - 1);
    } else {
        glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer);
...
void RenderPipeline::ClearBuffers() {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, colorTexture, 0);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depthBuffer);
    checkGlError("glBindFramebuffer 1");
